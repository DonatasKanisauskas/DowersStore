...
function Header() {
  ...
  const handleCategoriesDropDown = () => {
    let ele: HTMLElement | null = document.querySelector("div.navbar_dropdown");
    if (ele) {
      ele.style.bottom = categoriesDropDown ? "-193px" : "-304px";
      setNavDropDown(true);
    }
  ...
  const filterCategories = () => {
    let input = (
      document.querySelector(".categories_input") as HTMLInputElement
    ).value.toUpperCase();
    let a: NodeListOf<HTMLElement> =
      document.querySelectorAll(".categories_list a");

    Array.from(a).forEach((ele) => {
      let text =
        ele?.textContent?.toUpperCase() || ele?.innerText?.toUpperCase();
    ...
    <div className="head">
      <div className="navbar flex" style={{ top: showHeader ? "-50px" : "0" }}>
        <div className="navbar_mobile">
          <Link className="navbar_button title inline noShrink" to="/">
            React Shop
          </Link>
          <button
            className="navbar_button_dropdown"
        ...
        <div className="navbar_dropdown flex grow space-between">
          <div className="navbar_dropdown_section inline">
            <a
              className="navbar_button categories_button inline"
            ...
            <Link className="navbar_button inline" to="#link2">
              Link2
            </Link>
            <Link className="navbar_button inline" to="#link3">
              Link3
            </Link>
          </div>
          <div className="navbar_dropdown_section inline">
            <Link className="navbar_button inline" to="#user">
              User
            </Link>
            <Link className="navbar_button inline" to="/cart">
              Cart
            </Link>
          </div>
        </div>
